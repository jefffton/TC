...
function analyzeAllTestCases(testCases) {
  ...
  return {
    summary: {
      ...
      aiDetectedCount: results.filter(r => r.aiDetected).length,
    },
    results,
  };
}
...
function analyzeTestCase(tc, index) {
  ...
  const aiResult = detectAIContent(tc);
  criteria.push({ name: 'Originality (AI Detection)', score: aiResult.score, maxScore: 10, weight: 1, feedback: aiResult.feedback });
...
function detectAIContent(tc) {
  let aiSignals = 0;
  const feedback = [];
  const allText = [
    tc.title || '', tc.description || '', tc.objective || '',
    ...(Array.isArray(tc.steps) ? tc.steps.map(s => typeof s === 'string' ? s : s.action || '') : [tc.steps || '']),
    tc.expectedResult || tc.expected || '',
  ].join(' ').toLowerCase();

  const strongAIPhrases = [
    /\bseamlessly\b/, /\bin a timely manner\b/, /\bensure seamless\b/,
    /\boptimal performance\b/, /\benhance the user experience\b/,
    /\baforementioned\b/, /\bnotwithstanding\b/, /\bpertaining to\b/,
    /\bfacilitate\b/, /\bleverage\b/, /\bsubsequently\b/, /\bfurthermore\b/,
    /\bcomprehensive validation\b/, /\brobust and comprehensive\b/,
    /\bensure that the system seamlessly\b/, /\binitiate the .+ process\b/,
    /\bin the respective fields\b/, /\bthe appropriate dashboard\b/,
  ];

  const matchedPhrases = strongAIPhrases.filter(p => p.test(allText));
  aiSignals += matchedPhrases.length;

  if (Array.isArray(tc.steps) && tc.steps.length >= 5 && aiSignals >= 1) {
    const stepTexts = tc.steps.map(s => typeof s === 'string' ? s : (s.action || ''));
    const avgLen = stepTexts.reduce((sum, s) => sum + s.length, 0) / stepTexts.length;
    const lenVariance = stepTexts.reduce((sum, s) => sum + Math.pow(s.length - avgLen, 2), 0) / stepTexts.length;
    if (lenVariance < 15 && avgLen > 30) { aiSignals += 1; }
  }

  const isAI = aiSignals >= 3;
  let score;
  if (aiSignals === 0) { score = 10; feedback.push('Content appears human-written'); }
  else if (aiSignals <= 1) { score = 9; feedback.push('Looks human-written'); }
  else if (aiSignals <= 2) { score = 7; feedback.push('Some formal phrasing (likely fine)'); }
  else if (aiSignals <= 4) { score = 4; feedback.push('⚠️ Likely AI-generated'); if (matchedPhrases.length) feedback.push(`Flagged: "${matchedPhrases.slice(0,3).map(p=>p.source.replace(/\\b/g,'')).join('", "')}"`); }
  else { score = 2; feedback.push('🚨 Strongly appears AI-generated'); if (matchedPhrases.length) feedback.push(`Flagged: "${matchedPhrases.slice(0,3).map(p=>p.source.replace(/\\b/g,'')).join('", "')}"`); }

  return { score, feedback, isAI };
}
